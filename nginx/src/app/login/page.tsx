...
export default function Login() {
    ...
    const handleLogin = async (loginProvider) => {
        try {
            console.log("로그인 시도 중 - provider:", loginProvider);
            localStorage.setItem("provider", loginProvider);

            // 로그인 후, provider를 URL 파라미터로 전달하여 리다이렉트
            await signIn(loginProvider, { redirect: false });
    ...
    useEffect(() => {
        if (token) {
            // 로컬스토리지에서 provider 값을 불러오기
            const storedProvider = localStorage.getItem("provider");
            console.log("업데이트된 토큰:", token, "provider:", storedProvider);
            if (storedProvider) {
                sendTokenToBackend(token, storedProvider);
            }
            else{
                console.log("1111")
            }
    ...
    // 백엔드로 토큰 전송 함수
    const sendTokenToBackend = async (accessToken, provider) => {
        try {
            // provider에 따라 URL 분기
            const endpoint = provider === "google"
                ? "http://localhost:8080/api/auth/google/token"
                : `http://localhost:8080/api/auth/kakao`;
            console.log("🛠️ 백엔드로 전송할 accessToken:", accessToken);
            ...
            if (response.status >= 200 && response.status < 300) {
                console.log("토큰 전송 성공:", response.data);
            } else {
